...
def compute_average_distance_for_knn(emb, query, knn):
    """
    Compute the average distance of the `knn` nearest neighbors
    for a given set of embeddings and queries without Faiss.
    """
    bs = 1024  # Batch size
    all_distances = []
    
    # Normalize embeddings for cosine similarity (optional, depending on your use case)
    emb_norm = torch.nn.functional.normalize(emb, p=2, dim=1)
    query_norm = torch.nn.functional.normalize(query, p=2, dim=1)
    
    emb = emb_norm.transpose(0, 1).contiguous()
    
    for i in range(0, query_norm.shape[0], bs):
        # Compute the cosine similarity between the query batch and all embeddings
        distances = query_norm[i:i + bs].mm(emb)
        # Get the top `knn` nearest neighbors (largest cosine similarity means nearest)
        best_distances, _ = distances.topk(knn, dim=1, largest=True, sorted=True)
        all_distances.append(best_distances.mean(1).cpu())
    
    # Concatenate and return the mean of the distances
